...
func (app *Application) AddToCart() gin.HandlerFunc{
	return func(c *gin.Context) {
		...
		if productQueryID == "" {
			log.Println("Empty product ID")
			c.AbortWithError(http.StatusBadRequest, errors.New("empty product ID"))
			return
		}
		userQueryID := c.Query("userID")
		if userQueryID == "" {
			log.Println("Empty user ID")
			c.AbortWithError(http.StatusBadRequest, errors.New("empty user ID"))
			return
		}
...
func (app *Application) InstantBuy() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.IndentedJSON(200, "succesfully Product bought")
	}
}
