...
func RemoveProductFromCart(ctx context.Context, prodCollection *mongo.Collection, userCollection *mongo.Collection,productID primitive.ObjectID, userID string) error{
	id, err := primitive.ObjectIDFromHex(userID)
	if err != nil {
		log.Println(err)
		return ErrUserIdIsNotValid
...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string)error{
	...
	for _, user_item := range getusercart {
		var price = user_item["total"]
		total_price = price.(int32)
	}

	ordercart.Price = float64(total_price)
	filter := bson.D{primitive.E{Key: "_id", Value: id}}
...
func InstantBuyer(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, UserID string)error{
	...
	orders_detail.Price = float64(product_details.Price)
	filter := bson.D{primitive.E{Key: "_id", Value: id}}
